...
export default async function ProCirclePage({
  ...
  if (!user) redirect("/sign-in?next=/pro");
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="border-b px-4 py-3 flex items-center justify-between">
        <div>
          <Link href="/pro" className="text-sm text-muted-foreground">
            ← {t("back")}
          </Link>
      ...
      <div className="px-4">
        <Card className="p-4 mb-4 text-sm">
          <p className="font-medium mb-1">{t("dualInviteLink")}</p>
          <p className="text-muted-foreground mb-2">{t("dualInviteHint")}</p>
          <code className="block break-all rounded bg-muted p-2 text-xs">{caseLink}</code>
        </Card>

        <Card className="p-4 mb-4 text-sm">
          <p className="font-medium mb-2">{t("members")}</p>
          {(members ?? []).map((m) => {
